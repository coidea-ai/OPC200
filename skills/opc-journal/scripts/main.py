...
def _build_parser() -> _NoExitParser:
    ...
    # init - customer_id is required
    p = subparsers.add_parser("init", help="Initialize journal")
    p.add_argument("--customer-id", required=True, help="Unique customer identifier")
...
def main(context: dict = None) -> dict:
    ...
    if context and customer_id and not any(a.startswith("--customer-id") for a in argv):
        args_dict["customer_id"] = customer_id

    resolved_customer_id = args_dict.get("customer_id")
    if not resolved_customer_id:
        return {
            "status": "error",
            "result": None,
            "message": "--customer-id is required. Please provide a unique customer identifier."
        }

    return module.run(resolved_customer_id, args_dict)
