#!/usr/bin/env python3
"""
OPC200 Agent 模拟数据生成器（修复版）
修复点：
1. POST → PUT（避免指标类型冲突，每次推送替换整个 job）
2. 添加 DELETE 清理功能
3. 改进错误输出
"""
import time
...
def generate_metrics(tenant_id, os_type="linux", is_online=True):
    """生成 Prometheus 格式指标"""
    if not is_online:
        return ""
...
def main():
    parser = argparse.ArgumentParser(description='OPC200 Agent 模拟数据生成器')
    ...
    parser.add_argument('--delete', '-d', action='store_true', help='删除该 tenant 的指标')
    parser.add_argument('--os', default='linux', choices=['linux', 'windows', 'darwin'])
